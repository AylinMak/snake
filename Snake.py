...
def repaint():
    ...
    for body in snake_body:
        pygame.draw.circle(window, pygame.Color(0, 100, 50), (body[0], body[1]), scale/2)
        pygame.draw.rect(window, pygame.Color(10,0,150), pygame.Rect(food_position[0]-scale/2, food_position[1]-scale/2, scale, scale))

def game_over_massage():
    font = pygame.font.SysFont('Comic', scale*3)
    render = font.render(f"Game Over :( score: {score}", True, pygame.Color(255,0,0))
...
def paint_hud():
    font = pygame.font.SysFont("Comic", scale*2)
    render = font.render(f"Score: {score}", True, pygame.Color(100,255,100))
    rect = render.get_rect()
    window.blit(render, rect)
    pygame.display.flip()
